...
indir <- "C:/Users/bhadra9/OneDrive - unige.ch (1)/Project_EEG-BCI/Second_paper_data+code/06_BCI_online_CV_acc/CV_online_Colors/Data"
...
# write.csv(combined_data, file.path(file_path, "Online_colors_CVacc.csv"))
...
avgp <- ggplot() + 
  ...
  geom_errorbar(data = AVGSE, aes(x = Day, y = Accuracy, ymin = Accuracy - se, ymax = Accuracy + se), 
                width = .0, position = pd, size = 1.5, show.legend = FALSE, color = rgb(0, 102, 204, maxColorValue = 255)) +
  geom_line(data = AVGSE, aes(x = Day, y = Accuracy), position = pd, size = 2, color = rgb(0, 102, 204, maxColorValue = 255)) + 
...
avgp + theme(axis.line = element_line(size = 1), panel.border = element_blank(),
  ...
  scale_y_continuous(breaks = seq(52, 64, by = 2)) + # Set the tick interval to 10
  theme_SM
