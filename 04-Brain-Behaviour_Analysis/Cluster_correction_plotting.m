...
indir_rand   = 'C:\Users\bhadra9\OneDrive - unige.ch (1)\BCI_Group_shared\Code\KB\00_Reproducable_abstract_plots\Fig3_Brain-Behaviour\Output_data\Resampled_Data_v4\';
indir_obs    = 'C:\Users\bhadra9\OneDrive - unige.ch (1)\BCI_Group_shared\Code\KB\00_Reproducable_abstract_plots\Fig3_Brain-Behaviour\Output_data\Observed_Datav4\';
outdir_plots = 'C:\Users\bhadra9\OneDrive - unige.ch (1)\BCI_Group_shared\Code\KB\00_Reproducable_abstract_plots\Fig3_Brain-Behaviour\Plots\';


% List the csv files:
cd(indir_rand)
data_list_rand = dir('*t_val_pow*.csv'); 

cd(indir_obs)
data_list_obs = dir('*Agg*.csv'); 
...
for i_freq = 1: length(data_list_rand)
    ...
    title_name = strrep(freq_name ,'_','.') ;
   
    
    statrnd = table2array(datarand(:,3:end)); % the matrix should be 62chan by nrands
    statobs = table2array(dataobs(:,14))'; % the matrix should 1 by 62chan % 
...
%     cfg_topoTFR.colormap           = flip(tmpcolormap,1); % tmpcolormap; 
    ...
    cfg_topoTFR.zlim = [-6 6]; %[-max(input_data) max(input_data)]; %'maxabs'; %'maxmin', 'maxabs', 'zeromax', 'minzero', or [zmin zmax]
    ...
    c = colorbar;
    c.FontName = 'Arial';  c.FontSize = 20;
    c.Label.String   = 't-test';
    c.Label.FontSize = 22;
    
    % save the corrected
%     saveas(f,fullfile([outdir_plots,['Brain-evo_new_countour_corr6', freq_name, 'interaction.jpeg']]));
%     saveas(f,fullfile([outdir_plots,['Brain-evo_new_countour_corr6', freq_name, 'Pow-perf.eps']]));
